...
class EndDialogV2(private val baseActivity: BaseActivity, val mList: ArrayList<EndListInfo>?) :
    DialogFragment() {
...
//            binding.dotsIndicator.attachTo(binding.vpEnd)
    ...
    inner class Adapter : RecyclerView.Adapter<Adapter.ViewHolder>() {
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
            val binding =
                ItemEndItemV2Binding.inflate(LayoutInflater.from(baseActivity), parent, false)
        ...
        inner class ViewHolder(private val binding: ItemEndItemV2Binding) :
            ...
            fun bind(item: EndListInfo) {
                try {
                    ...
                    val round = Util.getPixel(baseActivity, 1f)
                    Glide.with(baseActivity).load(item.pGImg)
                        .transform(CenterCrop(), GranularRoundedCorners(round, round, 0f, 0f))
                        .into(binding.ivImg)
